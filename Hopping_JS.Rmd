...
ggplot(data = Hop, aes(x = TestTemp, y = Mean, col = Site, linetype = Sex)) +
  ...
  stat_summary(fun.y = mean, fun.ymin = std.err, fun.ymax = std.err, na.rm = TRUE, size = 1, position = position_dodge(width = 3)) +
...
#Do we really need 1|variable? It's less important than instar but they might get more tired after a few hops so order probably still matters... don't know if it needs to be nestsed within anything else (and hey, as I have it, it isn't)
