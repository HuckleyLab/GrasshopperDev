...
#what I did -- similar but uses glmer and compares AIC

#all interaction terms
MS0 <- glmer(
  Survive ~ sex * Site * Temp * Light + 
    (1 | Female2), 
  data = Instars, 
  family = binomial(link = "logit")
)

#All 1st and 2nd order + Site:Light:Temp
MS1 <- glmer(
  Survive ~ sex + Site + Temp + Light + 
    sex:Site + sex:Temp + sex:Light + Site:Temp + Site:Light + Temp:Light + 
    Site:Light:Temp + 
...
#Site*Temp*Light (no sex)
MS4 <- glmer(
  Survive ~ Site + Temp + Light + 
    Site:Temp + Site:Light + Temp:Light + 
    Site:Temp:Light + 
    (1 | Female2), 
  data = Instars, 
...
#Rory's MS5 was a duplicate of MS4

#All first and second order except sex:Temp
MS6 <- glmer(
  Survive ~ sex + Site + Temp + Light + 
    sex:Site + sex:Light + Site:Temp + Site:Light + Temp:Light + 
    (1 | Female2), 
...
#1st and second order of site temp and light, just first order sex 
MS7 <- glmer(
  Survive ~ sex + Site + Temp + Light + 
    Site:Temp + Site:Light + Temp:Light + 
    (1 | Female2), 
...
#first and second order site temp light
MS8 <- glmer(
  Survive ~ Site + Temp + Light + 
    Site:Temp + Site:Light + Temp:Light + 
    (1 | Female2), 
...
#all first order, second: just site:temp and temp:light
MS9 <- glmer(
  Survive ~ sex + Site + Temp + Light + 
    Site:Temp + Temp:Light + 
    (1 | Female2), 
...
#all first order, second: just site:temp
MS10 <- glmer(
  Survive ~ sex + Site + Temp + Light + 
    Site:Temp + 
    (1 | Female2), 
...
#1st order: omit light, second: just site:temp
MS11 <- glmer(
  Survive ~ sex + Site + Temp + 
    Site:Temp + 
    (1 | Female2), 
  data = Instars, 
  family = binomial(link = "logit")
...
aictab(list(MS0, MS1, MS2, MS3, MS4, MS6, MS7, MS8, MS9, MS10, MS11), modnames = c("MS0", "MS1", "MS2", "MS3", "MS4", "MS6", "MS7", "MS8", "MS9", "MS10", "MS11"))

#a lot of comparable models in terms of AIC

#MS11 -- sex + Site + Temp + Site:Temp... delta AICc 0... nested in MS9, MS10
#MS9 -- sex + Site + Temp + Light + Site:Temp + Temp:Light... delta AICc 1.41 -- elim
#MS10 -- sex + Site + Temp + Light + Site:Temp... delta AICc 1.87, nested in MS9 -- elim
#MS8 Site + Temp + Light + Site:Temp + Site:Light + Temp:Light... delta AICc 2.28 -- less competitive bc >2

#nothing quite significant -- no p values under .05
Anova(MS11, type = 3) 
summary(MS11) 

##for MS8 there's one p value in the summary that is <.05 (SiteB1:TempLV significantly higher)
#Anova(MS8, type = 3) #nothing quite significant -- no p values under .05
#summary(MS8)
...
```{r}
Instar_death_test <- Instars[which(Instars$sex %in% c("F","M")),]

#coding Death_Instar adult as 6... looking at effect on 
Instar_death_test$Death_Instar[Instar_death_test$Death_Instar=="A"]=6 
Instar_death_test$Death_Instar <- as.numeric(Instar_death_test$Death_Instar)

hist(as.numeric(Instar_death_test$Death_Instar))
```
...
ggplot(data = Instar_death_test, aes(x = Site, y = Death_Instar, color = Temp)) +
#  rory_theme +
...
```
```{r}
#TRY THIS PLOT TO HIGLIGHT VAR< DAYLENGTH
ggplot(data = Instars, aes(x = Site, y = Age_Adult, color = Temp, linetype=Light)) +
#  rory_theme +
  facet_grid(cols=vars(sex), labeller=labeller(sex = sex.labs, Light = Light.labs)) +
  stat_summary(fun.y = mean, fun.ymin = MinSE, fun.ymax = MaxSE, na.rm = TRUE, size = .5, position = position_dodge(width = .3)) +
  stat_summary(aes(x = as.numeric(as.factor(Site))), fun.y = mean, geom = "line", size = .5, position = position_dodge(width = .3)) +
  #stat_summary(fun.data = Give.N, geom = "text", fun.y = median, position = position_dodge(width = .6), size = 7) +
  ylab("Time to adult (d)") + xlab("Site")+scale_color_manual(name="Temperature", labels=c("High var", "Low var"), values = c("#D6604D", "#4393C3")) + scale_x_discrete(labels=c("2195m", "2591m", "3048m"))+ theme(axis.text.x = element_text(angle = 90, vjust = .2, hjust=1))
...
DON'T NEED TO RUN OLD ALTERNATIVE
### OLD ALTERNATIVE
```{r}
...
```{r}
ggplot(data = Instars_names, aes(x = Site, y = as.numeric(Mass_Adult), color = Temperature, lty=Photoperiod)) +
#  rory_theme +
  facet_grid(.~Sex, labeller=labeller(Sex = sex.labs, Photoperiod = Light.labs)) +
  stat_summary(fun.y = mean, fun.ymin = MinSE, fun.ymax = MaxSE, na.rm = TRUE, size = .5, position = position_dodge(width = .3)) +
  stat_summary(aes(x = as.numeric(as.factor(Site))), fun.y = mean, geom = "line", size = .5, position = position_dodge(width = .3)) +
  #stat_summary(fun.data = Give.N, geom = "text", fun.y = median, position = position_dodge(width = .6), size = 7) +
  ylab("Mass as adult (g)") + xlab("Site")+scale_color_manual(name="Temperature", labels=c("High var", "Low var"), values = c("#D6604D", "#4393C3")) + scale_x_discrete(labels=c("2195m", "2591m", "3048m"))+ theme(axis.text.x = element_text(angle = 90, vjust = .2, hjust=1))
...
```{r}
Instars_names_devmass <- Instars_names %>% 
...
ggplot(data = Instars_names_devmass, aes(x = Site, y = y, color = Temperature, linetype=Sex)) +
  facet_grid(meas~Photoperiod, scales="free_y", labeller=labeller(Photoperiod = Light.labs), switch="y") +
  stat_summary(fun.y = mean, fun.ymin = MinSE, fun.ymax = MaxSE, na.rm = TRUE, size = .5, position = position_dodge(width = .3)) +
  stat_summary(aes(x = as.numeric(as.factor(Site))), fun.y = mean, geom = "line", size = .5, position = position_dodge(width = .3)) +
  xlab("Site")+scale_color_manual(name="Temperature", labels=c("High var", "Low var"), values = c("#D6604D", "#4393C3")) + scale_linetype_discrete(labels=c("Female", "Male")) + scale_x_discrete(labels=c("2195m", "2591m", "3048m"))+ theme(axis.text.x = element_text(angle = 90, vjust = .2, hjust=1), axis.title.y = element_blank(), strip.background.y = element_blank(),  strip.placement = "outside") 
...
```{r}
hist(as.numeric(Instars$Mass_Adult), breaks = 10)
```
...
#^THIS IS AN ISSUE! SEE analysis_concise_clean.Rmd
```
...
Lmer0.2 <- lmer(as.numeric(value) ~ Sex * Site * Temperature * Photoperiod +
                   (1|Female2/Individual)+(1|Instar)
                , na.action = 'na.omit', REML=FALSE, data = M_Instars_Adult_Long)
#^ not sure why I tried this -- I want to see interactions with instar, and this won't show me that
...
aictab(list(Lmer0.1, Lmer0.3, Lmer0.4, Lmer0.5), modnames = c("Lmer0.1", "Lmer0.3", "Lmer0.4", "Lmer0.5"))

#once again AICs are worse than treating instar as categorical

#I go with Lmer0.1 anyway to be consistent with dev instar analysis
...
#hmm so possible issue with this plot is if I don't control for the different treatments
#the it could actually be a series of not linear relationships (like for example a series of blobs)
...
# Ctmin and Ctmax -- sorry it's a bit of a mess
```{r, echo=FALSE}
...
Lmermax_t <- lmer(as.numeric(CT_max) ~ Temperature +
                   (1|Female),
                 REML = FALSE,
                 na.action = 'na.omit', data = CT)
Lmermax_l <- lmer(as.numeric(CT_max) ~ Photoperiod +
                   (1|Female),
                 REML = FALSE,
                 na.action = 'na.omit', data = CT)

Lmermax_tl <- lmer(as.numeric(CT_max) ~ Temperature + Photoperiod +
                   (1|Female),
                 REML = FALSE,
                 na.action = 'na.omit', data = CT)
#singular

Lmermax_txl <- lmer(as.numeric(CT_max) ~ Temperature * Photoperiod +
                   (1|Female),
                 REML = FALSE,
                 na.action = 'na.omit', data = CT)
#singular
...
Lmermin_t <- lmer(as.numeric(CT_min) ~ Temperature +
                   (1|Female),
                 REML = FALSE,
                 na.action = 'na.omit', data = CT)
Lmermin_l <- lmer(as.numeric(CT_min) ~ Photoperiod +
                   (1|Female),
                 REML = FALSE,
                 na.action = 'na.omit', data = CT)

Lmermin_tl <- lmer(as.numeric(CT_min) ~ Temperature + Photoperiod +
                   (1|Female),
                 REML = FALSE,
                 na.action = 'na.omit', data = CT)
#singular
...
```{r}
#not sure about whether to do txl or t+l... this would indicate t+l or even simply l... but might want to go with the same model for both

lm_txl <- lm(cbind(as.numeric(CT_min), as.numeric(CT_max)) ~ Temperature * Photoperiod,
                 na.action = 'na.omit', data = CT)
lm_tl <- lm(cbind(as.numeric(CT_min), as.numeric(CT_max)) ~ Temperature + Photoperiod,
                 na.action = 'na.omit', data = CT)
lm_t <- lm(cbind(as.numeric(CT_min), as.numeric(CT_max)) ~ Temperature,
                 na.action = 'na.omit', data = CT)
lm_l <- lm(cbind(as.numeric(CT_min), as.numeric(CT_max)) ~ Photoperiod,
                 na.action = 'na.omit', data = CT)
...
a <- a %>% unite(p, c(`Pr(>F)`, Significance), sep="")
a[5,4] <- NA
b <- Anova(lm_txl_max, type=3)
rownames(b) <- c()
b$Significance <- "   "
b$Significance[b$`Pr(>F)`<.05] <- "*  "
b$Significance[b$`Pr(>F)`<.001] <- "***"
b$`Pr(>F)` <- format(b$`Pr(>F)`, scientific=TRUE, digits=3)
b$`Pr(>F)` <- as.character(b$`Pr(>F)`)
b <- b %>% unite(p, c(`Pr(>F)`, Significance), sep="")
b[5,4] <- NA
a <- cbind(a,b)
options(knitr.kable.NA = '')
kable(a, digits=3, col.names=c("Sum Sq", "df", "F value", "p", "Sum Sq", "df", "F value", "p"), escape=FALSE, format="html") %>% kable_styling() %>% add_header_above(c(" " = 1, "CTmin" = 4, "CTmax" = 4)) %>% save_kable("CTminmaxanova.png")
...
#   ylab("Time to adult (d)") + xlab("Site")+scale_color_manual(name="Temperature", labels=c("High var", "Low var"), values = c("#D6604D", "#4393C3")) + scale_x_discrete(labels=c("2195m", "2591m", "3048m"))+ theme(axis.text.x = element_text(angle = 90, vjust = .2, hjust=1))
```
Visualize CTmin and CTmax together (putting sexes together on a plot)
```{r}
Instars_names_devmass <- Instars_names %>% 
  rename(`Time to adult (days)` = Age_Adult, `Mass as adult (g)` = Mass_Adult) %>%
  gather(key="meas", value="y", `Time to adult (days)`, `Mass as adult (g)`)
#^I think the warning is ok. We want mass to be numeric / age can be also
...
ggplot(data = CTminmax, aes(x = Site, y = y, color = Temperature, linetype=Sex)) +
  facet_grid(meas~Photoperiod, scales="free_y", labeller=labeller(Photoperiod = Light.labs), switch="y") +
  stat_summary(fun.y = mean, fun.ymin = MinSE, fun.ymax = MaxSE, na.rm = TRUE, size = .5, position = position_dodge(width = .3)) +
  stat_summary(aes(x = as.numeric(as.factor(Site))), fun.y = mean, geom = "line", size = .5, position = position_dodge(width = .3)) +
  xlab("Site")+scale_color_manual(name="Temperature", labels=c("High var", "Low var"), values = c("#D6604D", "#4393C3")) + scale_linetype_discrete(labels=c("Female", "Male")) + scale_x_discrete(labels=c("2195m", "2591m", "3048m"))+ theme(axis.text.x = element_text(angle = 90, vjust = .2, hjust=1), axis.title.y = element_blank(), strip.background.y = element_blank(),  strip.placement = "outside") 

#add in preferred body temp
# ggplot(data = PBT %>% filter(!is.na(Sex)), aes(x = Site, y = Tb_Mean, color = Temp)) +
# #  rory_theme +
#   facet_grid(Sex~Light, labeller=labeller(Sex = sex.labs, Light = Light.labs)) +
#   stat_summary(fun.y = mean, fun.ymin = MinSE, fun.ymax = MaxSE, na.rm = TRUE, size = .5, position = position_dodge(width = .3)) +
...
```{r}
plot_model(lm_txl_min, type="pred", terms=c("Light", "Temp"), show.data=FALSE)
plot_model(lm_txl_min, type="pred", terms=c("Temp"), show.data=FALSE)
...
# might not be a big deal since it's a random effect in the model... I'll do both just in case
#(see below)
...
Hop_Long <- Hop_Long %>% rename(Temperature=Temp, Photoperiod=Light)

ggplot(data = Hop_Long, aes(x = TestTemp, y = value, col = Site)) +
  facet_grid(Temperature~Sex+Photoperiod, labeller=labeller(Temperature=Temp.labs, Photoperiod = Light.labs, Sex=sex.labs)) +
  stat_summary(fun = mean, fun.min = MinSE, fun.max = MaxSE, na.rm = TRUE, size = .5, position = position_dodge(width = .3)) +
  stat_summary(aes(x = as.numeric(TestTemp)), fun = mean, geom = "line", size = .5, position = position_dodge(width = .3)) + scale_color_manual(labels=c("2195m", "2591m", "3048m"), values = brewer.pal(n = 8, name = "Blues")[c(8,6,4)]) +
  #stat_summary(fun.data = Give.N, geom = "text", fun.y = median, position = position_dodge(width = .6), size = 7) +
  ylab("Hopping distance (cm)") + xlab("Test temperature")
```
...
LmerL_H0 <- lmer(value ~ poly(TestTemp, 2, raw = TRUE) * Sex * Site * Temperature * Photoperiod + 
...
```
```{r}
plot_model(LmerL_H0, type="pred", terms=c("TestTemp", "Photoperiod", "Sex"), show.data=FALSE)
```

```{r}
dat <- ggpredict(LmerL_H0, terms = c("TestTemp","Light", "Temp", "Site"), condition = c(Sex = "M"))
pM <- plot(dat, facet = TRUE)

dat <- ggpredict(LmerL_H0, terms = c("TestTemp","Light", "Temp", "Site"), condition = c(Sex = "F"))
pF <- plot(dat, facet = TRUE)

plot_grid(pM[[1]], pM[[2]], pM[[3]], pF[[1]], pF[[2]], pF[[3]])
```
...
```
```{r}
plot_model(LmerL_H0, type="pred", terms=c("TestTemp", "Temp"), show.data=FALSE)


```
```{r}
plot_model(LmerL_H0, type="pred", terms=c("TestTemp", "Sex", "Site"), show.data=FALSE)
```
```{r}
plot_model(LmerL_H0, type="pred", terms=c("TestTemp", "Sex", "Light"), show.data=FALSE)
```
```{r}
plot_model(LmerL_H0, type="pred", terms=c("TestTemp", "Temp", "Light"), show.data=FALSE)
```
```{r}
plot_model(LmerL_H0, type="pred", terms=c("TestTemp", "Temp", "Light"), show.data=FALSE)
```
...
ggplot(data = FeedPL, aes(x = TestTemp, y = P1_Consumed, col = Site)) +
  ...
  stat_summary(aes(x = as.numeric(TestTemp)), fun = mean, geom = "line", size = .5, position = position_dodge(width = .3)) + scale_color_manual(labels=c("2195m", "2591m", "3048m"), values = brewer.pal(n = 8, name = "Blues")[c(8,6,4)]) +
  #stat_summary(fun.data = Give.N, geom = "text", fun.y = median, position = position_dodge(width = .6), size = 7) +
  ylab("Leaf area consumed (cm2, 3h)") + xlab("Test temperature")
```
...
```
ANOVA Table for the best performing one, 3.5 (sticking w REML=FALSE... but not actually?)
```{r}
...
```{r}
p <- ggpredict(LmerF3.5, terms = c("TestTemp", "Site", "Sex"))
plot(p, facet=TRUE)
...
```{r}
p <- ggpredict(LmerF3.5, terms = c("TestTemp", "Light"))
plot(p, facet=TRUE)
...
```{r}
ggpredict(LmerF3.5, terms = c("Site", "Light"))
```

```{r}
ggpredict(LmerF3.5, terms = c("TestTemp"))
```
Another thing to think about -- crop loading vs processing (not just total time)

### Mass-corrected -- read in Adults only bind thru Individual / FULL_ID; use Mass_Adult
```{r}
AOmass <- AdultsOnly %>% rename(FULL_ID = Individual) %>% select(FULL_ID, Mass_Adult)
FeedPLmass <- merge(FeedPL, AOmass, by="FULL_ID") 
...
```{r}
ggplot(data = FeedPLmass, aes(x = TestTemp, y = Tot_Consumed_scaled, col = Site)) +
  facet_grid(Temperature~Sex+Photoperiod, labeller=labeller(Temperature=Temp.labs, Photoperiod = Light.labs, Sex=sex.labs)) +
  stat_summary(fun = mean, fun.min = MinSE, fun.max = MaxSE, na.rm = TRUE, size = .5, position = position_dodge(width = .3)) +
  stat_summary(aes(x = as.numeric(TestTemp)), fun = mean, geom = "line", size = .5, position = position_dodge(width = .3)) + scale_color_manual(labels=c("2195m", "2591m", "3048m"), values = brewer.pal(n = 8, name = "Blues")[c(8,6,4)]) +
  #stat_summary(fun.data = Give.N, geom = "text", fun.y = median, position = position_dodge(width = .6), size = 7) +
  ylab("Mass-scaled leaf area consumed (cm2/g, 8h)") + xlab("Test temperature")



ggplot(data = FeedPLmass, aes(x = TestTemp, y = P1_Consumed_scaled, col = Site)) +
  facet_grid(Temperature~Sex+Photoperiod, labeller=labeller(Temperature=Temp.labs, Photoperiod = Light.labs, Sex=sex.labs)) +
  stat_summary(fun = mean, fun.min = MinSE, fun.max = MaxSE, na.rm = TRUE, size = .5, position = position_dodge(width = .3)) +
  stat_summary(aes(x = as.numeric(TestTemp)), fun = mean, geom = "line", size = .5, position = position_dodge(width = .3)) + scale_color_manual(labels=c("2195m", "2591m", "3048m"), values = brewer.pal(n = 8, name = "Blues")[c(8,6,4)]) +
  #stat_summary(fun.data = Give.N, geom = "text", fun.y = median, position = position_dodge(width = .6), size = 7) +
  ylab("Mass-scaled leaf area consumed (cm2/g, 3h)") + xlab("Test temperature")
```
```{r}
...
```
ANOVA Table for the best performing one, 3.5 (sticking w REML=FALSE)
```{r}
a <- Anova(LmerF3.5, type=3)
a$Significance <- "   "
a$Significance[a$`Pr(>Chisq)`<.05] <- "*  "
...
```{r}
#I get a weird error "non-conformable arguments"
...
#useful stackoverflow: https://stackoverflow.com/questions/34221564/non-conformable-arguments-error-from-lmer-when-trying-to-extract-information-fro
dat <- ggpredict(LmerF3.5, terms = c("TestTemp","Site", "Sex"))
pM <- plot(dat, facet = TRUE)
```

```{r}
plot_model(LmerF3.5, type= "pred", terms = c("TestTemp","Site", "Sex"), show.data=FALSE)
```
```{r}
plot_model(LmerF3.5, type= "pred", terms = c("TestTemp", "Sex"), show.data=FALSE)
```
```{r}
LmerF0 <- lmer(Tot_Consumed_scaled ~ poly(TestTemp, 2, raw = TRUE) * Sex * Site * Temperature * Photoperiod + 
                 (1|Mom/ID), 
...
```
```{r}
plot_model(LmerF3.4, type= "pred", terms = c("Temperature", "Site"), show.data=FALSE)
```
...
plot_model(nLmer0, type= "pred", terms = c("Site","Temp","Light","sex"), show.data=FALSE, show.loess=TRUE)
