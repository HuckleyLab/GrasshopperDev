...
Photoperiod.labs <- c("Long-day", "Short-day")
names(Photoperiod.labs) <- c("L", "S")

#CHANGED
Pplabsnew <- c(L="long-day", S="short-day")
...
#CHANGED 6/16
p.devmass <- ggplot(data = Instars_names_devinstmass, aes(x = Site, y = y, color = Temperature, linetype=Photoperiod, shape=Photoperiod)) +
  facet_grid(meas~Sex, scales="free_y", labeller=labeller(Photoperiod = Photoperiod.labs), switch="y") +
  ...
  xlab("Site")+scale_color_manual(values = c("#D6604D", "#4393C3")) + scale_shape_manual(values=c(16,1), labels = c("Long-day", "Short-day")) + scale_linetype_discrete(labels = c("Long-day", "Short-day"))+ 
  theme_bw() + theme(strip.background = element_blank(), axis.text.x = element_text(angle = 0, vjust = .2, hjust=.5), axis.title.y = element_blank(), strip.placement = "outside")

p.devmass
#ggsave(p.devmass, file="Figures/Fig1_devmass.pdf")
#DONE


#calculate Ns
#sums to 183 
#sum(is.na(Instars_names_devinstmass$y) & Instars_names_devinstmass$meas=="Time to adult (d)")  
#sum(is.na(Instars_names_devinstmass$y) & Instars_names_devinstmass$meas=="Mass as adult (g)")
s <- Instars_names_devinstmass %>% filter(!is.na(y), meas=="Mass as adult (g)") %>% group_by(Temperature, Photoperiod, Site, Sex) %>% summarise(N=n())
sum(s$N)
...
#how many different mothers (per site)?
Instars_names %>% group_by(Site) %>% summarise(nF=length(unique(Female2)))
```
...
#ggsave(p.devmass, file="Figures/Fig1_devmass.pdf")
#DONE
# ggplot(data = Instars, aes(x = Site, y = Age_Adult, color = Temp, linetype=sex)) +
# #  rory_theme +
#   facet_grid(cols=vars(Light), labeller=labeller(sex = sex.labs, Light = Light.labs)) +
...
```{r}
# 4 way interaction
#plot_model(devmod,type="pred",terms=c("Site","Temperature","Photoperiod","Sex"), show.data=FALSE, show.legend=FALSE) #%>% 

```
Nicer
```{r}
# ggplot(data = Instars_names, aes(x = Site, y = Age_Adult, color = Temperature, linetype=Photoperiod, shape=Photoperiod)) +
# #  rory_theme +
#   facet_grid(cols=vars(Sex), labeller=labeller(Photoperiod = Photoperiod.labs)) +
#   stat_summary(fun = mean, fun.min = MinSE, fun.max = MaxSE, na.rm = TRUE, size = .5, position = position_dodge(width = .3)) +
#   stat_summary(aes(x = as.numeric(as.factor(Site))), fun = mean, geom = "line", size = .5, position = position_dodge(width = .3)) +
#   xlab("Site")+scale_color_manual(values = c("#D6604D", "#4393C3")) + scale_shape_manual(values=c(16,1), labels = c("Long-day", "Short-day")) + scale_linetype_discrete(labels = c("Long-day", "Short-day"))+
#   theme_bw()

mydf <- ggpredict(devmod,terms=c("Site","Temperature","Photoperiod","Sex"))
mydf <- mydf %>% rename(Site=x, Temperature=group, Sex=panel, Photoperiod=facet)
p.devpred <- ggplot(data = mydf, aes(x = Site, y = predicted, color = Temperature, linetype=Photoperiod, shape=Photoperiod)) +
#  rory_theme +
  facet_grid(cols=vars(Sex), labeller=labeller(Photoperiod = Photoperiod.labs)) + 
  geom_line(aes(group=interaction(Sex, Photoperiod, Temperature)), position = position_dodge(width = .3)) + geom_point(size=2, position = position_dodge(width = .3)) + geom_errorbar(aes(ymin=conf.low,ymax=conf.high), width=.3, position = position_dodge(width = .3)) +
  #stat_summary(fun = predicted, fun.min = conf.low, fun.max = conf.high, na.rm = TRUE, size = .5, position = position_dodge(width = .3)) +
  #stat_summary(aes(x = as.numeric(as.factor(Site))), fun = Site, geom = "line", size = .5, position = position_dodge(width = .3)) +
  xlab("Site")+ ylab("Predicted time to adult (d)")+ scale_color_manual(values = c("#D6604D", "#4393C3")) + scale_shape_manual(values=c(16,1), labels = c("Long-day", "Short-day")) + scale_linetype_discrete(labels = c("Long-day", "Short-day"))+
  theme_bw() +theme(strip.background = element_blank(), axis.text.x = element_text(angle = 0, vjust = .2, hjust=.5))

p.devpred
ggsave(p.devpred, file="Figures/FigS1_devpred.png")
```
...
```
Brief digression: Is there a linear relationship between adult mass and adult development time? Nope!
```{r}
l <- lm(as.numeric(Mass_Adult) ~ as.numeric(Age_Adult), data=Instars_names)
summary(l)
```
...
```{r}
masscubrt <- Instars_names$Mass_Adult^(1/3)
fem2 <- lm(Instars_names$Femur_Adult ~ masscubrt)
summary(fem2)

pro2 <- lm(Instars_names$Pronotum_Adult ~ masscubrt)
summary(pro2)
```
...
## Fig S3
```{r}
mydf <- ggeffect(massmod,terms=c("Site","Temperature","Sex"))
mydf <- mydf %>% rename(Site=x, Temperature=group, Sex=facet)
p.masspredsig <- ggplot(data = mydf, aes(x = Site, y = predicted, color = Temperature)) +
#  rory_theme +
  facet_grid(cols=vars(Sex)) + 
  geom_line(aes(group=interaction(Sex, Temperature)), linetype="longdash", position = position_dodge(width = .3)) + geom_point(alpha=.5, size=2, position = position_dodge(width = .3)) + geom_errorbar(aes(ymin=conf.low,ymax=conf.high), width=.3, linetype="longdash", position = position_dodge(width = .3)) +
  #stat_summary(fun = predicted, fun.min = conf.low, fun.max = conf.high, na.rm = TRUE, size = .5, position = position_dodge(width = .3)) +
  #stat_summary(aes(x = as.numeric(as.factor(Site))), fun = Site, geom = "line", size = .5, position = position_dodge(width = .3)) +
  xlab("Site")+ ylab("Predicted mass as adult (g)")+ scale_color_manual(values = c("#D6604D", "#4393C3")) + 
  theme_bw() +theme(strip.background = element_blank(), axis.text.x = element_text(angle = 0, vjust = .2, hjust=.5))

p.masspredsig
#ggsave(p.masspredsig, file="Figures/FigS3_masspredsig.png")
```
## Fig S4
```{r}
mydf <- ggeffect(massmod,terms=c("Site","Temperature","Photoperiod"))
mydf <- mydf %>% rename(Site=x, Temperature=group, Photoperiod=facet)
p.masspredinsig <- ggplot(data = mydf, aes(x = Site, y = predicted, color = Temperature, linetype=Photoperiod, shape=Photoperiod)) +
#  rory_theme +
  facet_grid(labeller=labeller(Photoperiod = Photoperiod.labs)) + 
  geom_line(aes(group=interaction(Photoperiod, Temperature)), position = position_dodge(width = .3)) + geom_point(size=2, position = position_dodge(width = .3)) + geom_errorbar(aes(ymin=conf.low,ymax=conf.high), width=.3, position = position_dodge(width = .3)) +
  #stat_summary(fun = predicted, fun.min = conf.low, fun.max = conf.high, na.rm = TRUE, size = .5, position = position_dodge(width = .3)) +
  #stat_summary(aes(x = as.numeric(as.factor(Site))), fun = Site, geom = "line", size = .5, position = position_dodge(width = .3)) +
  xlab("Site")+ ylab("Predicted time to adult (d)")+ scale_color_manual(values = c("#D6604D", "#4393C3")) + scale_shape_manual(values=c(16,1), labels = c("Long-day", "Short-day")) + scale_linetype_discrete(labels = c("Long-day", "Short-day"))+
  theme_bw() +theme(strip.background = element_blank(), axis.text.x = element_text(angle = 0, vjust = .2, hjust=.5))
...
p.masspredinsig
ggsave(p.masspredinsig, file="Figures/FigS4_masspredinsig.png")
```
...
devanova$Significance[devanova$`Pr(>Chisq)`<.01] <- "** "
...
massanova$Significance[massanova$`Pr(>Chisq)`<.01] <- "** "
...
devcoeff$Significance[devcoeff$`Pr...t..`<.01] <- "** "
...
masscoeff$Significance[masscoeff$`Pr...t..`<.01] <- "** "
...
## IMPORTANT NOTE: S2 and S3 mass values are new! (old new, that is -6/16)
...
devinstanova$Significance[devinstanova$`Pr(>Chisq)`<.01] <- "** "
...
massinstanova$Significance[massinstanova$`Pr(>Chisq)`<.01] <- "** "
...
devinstcoeff$Significance[devinstcoeff$`Pr...t..`<.01] <- "** "
...
massinstcoeff$Significance[massinstcoeff$`Pr...t..`<.01] <- "** "
...
CTminmax <- CT %>% 
  #rename(`CTmin (°C)`= CT_min, `CTmax (°C)`= CT_max) %>%
  gather(key="meas", value="y", CT_min, CT_max)

PBTnew <- PBT %>% filter(!is.na(Sex)) %>% #WONDERING if I should exclude Sex=NA when PBT is loaded
  rename("PBT"= Tb_Mean) %>% gather(key="meas", value="y", PBT)

CTPBT <- merge(CTminmax, PBTnew, all = TRUE)


# neworder <- c("CTmax (°C)", "PBT (°C)", "CTmin (°C)")
neworder <- c("CT_max", "PBT", "CT_min")
CTPBT2 <- arrange(transform(CTPBT,
             meas=factor(meas,levels=neworder)),meas)


#new
fake_data1 <- expand.grid(meas=unique(CTPBT2$meas),Temperature=c("HV","LV"), Photoperiod=c("L", "S"), Site=c("2195m", "2591m", "3048m"), Sex=unique(CTPBT2$Sex)) %>% mutate(y=recode(meas, CT_max=55, PBT=45, CT_min=15))
fake_data2 <- expand.grid(meas=unique(CTPBT2$meas),Temperature=c("HV","LV"), Photoperiod=c("L", "S"), Site=c("2195m", "2591m", "3048m"), Sex=unique(CTPBT2$Sex)) %>% mutate(y=recode(meas, CT_max=40, PBT=30, CT_min=0))
fake_data <- rbind(fake_data1, fake_data2)


p.CTPBT <- ggplot(CTPBT2, aes(x= Site, y=y, color=Temperature, linetype=Photoperiod, shape=Photoperiod)) +
  facet_grid(meas~Sex, scales="free_y", switch="y") +
  stat_summary(fun = mean, fun.min = MinSE, fun.max = MaxSE, na.rm = TRUE, size = .5, position = position_dodge(width = .3)) +
  stat_summary(aes(x = as.numeric(as.factor(Site))), fun = mean, geom = "line", size = .5, position = position_dodge(width = .3)) +
...
```{r}
#Ns for fig 3
s <- CTPBT2 %>% filter(!is.na(y)) %>% group_by(Site, Temperature, Photoperiod, Sex) %>% summarise(CT_min=sum(meas=="CT_min"), PBT=sum(meas=="PBT"), CT_max=sum(meas=="CT_max"))
...
Need to merge into above / clean up
```{r}
# New facet label names for supp variable
meas.labs <- c("CTmax (°C)", "PBT (°C)", "CTmin (°C)")
names(meas.labs) <- c("CT_max", "PBT", "CT_min")

p.CTPBT <- ggplot(CTPBT2, aes(x= Site, y=y, color=Temperature, linetype=Photoperiod, shape=Photoperiod)) + geom_blank(data=fake_data, aes(x = Site, y = y, color = Temperature, linetype=Photoperiod, shape=Photoperiod)) +
   facet_grid(meas~Sex, scales="free_y", switch="y", labeller=labeller(meas=meas.labs)) +
   stat_summary(fun = mean, fun.min = MinSE, fun.max = MaxSE, na.rm = TRUE, size = .5, position = position_dodge(width = .3)) +
   stat_summary(aes(x = as.numeric(as.factor(Site))), fun = mean, geom = "line", size = .5, position = position_dodge(width = .3)) +
   xlab("Site")+scale_color_manual(values = c("#D6604D", "#4393C3")) + scale_shape_manual(values=c(16,1), labels = c("Long-day", "Short-day")) + scale_linetype_discrete(labels = c("Long-day", "Short-day"))+ theme_bw() + theme(strip.background = element_blank(), strip.placement = "outside", axis.text.x = element_text(angle = 0, vjust = .2, hjust=.5), axis.title.y = element_blank())
#ggsave(p.CTPBT, file="Figures/Fig3_CTPBT.pdf") #DONE
...
PBTanova$Significance[PBTanova$`Pr(>Chisq)`<.01] <- "** "
...
PBTcoeff$Significance[PBTcoeff$`Pr...t..`<.01] <- "** "
...
CTminanova$Significance[CTminanova$`Pr(>F)`<.01] <- "** "
...
CTmaxanova$Significance[CTmaxanova$`Pr(>F)`<.01] <- "** "
...
```{r}
#run these go get labels A and B (including save_kable)

kable(PBTanova, digits=1, col.names=c("<var>&chi;<sup>2</sup></var>", "df", "p"), escape=FALSE, format="html", align = 'clc', caption = 'A') %>% kable_styling() #%>% save_kable("Tables/Tab2_PBTanova.png")

kable(CTminmaxanova, digits=1, col.names=c("Sum Sq", "df", "F value", "p", "Sum Sq", "df", "F value", "p"), escape=FALSE, format="html", align = 'clc', caption = 'B') %>% kable_styling() %>% add_header_above(c(" " = 1, "CTmin" = 4, "CTmax" = 4)) #%>% save_kable("Tables/Tab3_CTminmaxanova.png")

```

```{r}
##image_border not recognized for some reason, but it worked before
#im2 <- image_border(image_read("Tables/Tab2_PBTanova.png"), "white", "0x20")
#im3 <- image_border(image_read("Tables/Tab3_CTminmaxanova.png"), "white", "0x20")


#img <- c(im2, im3)
#tab2and3 <- image_append(img, stack = TRUE)

#image_write(tab2and3, path = "Tables/Tab2n3_PBTCTminmaxanova.png", format = "png")
...
CTmincoeff$Significance[CTmincoeff$`Pr...t..`<.01] <- "** "
...
CTmaxcoeff$Significance[CTmaxcoeff$`Pr...t..`<.01] <- "** "
...
Tables still incomplete (I THINK THIS IS AN OLD COMMENT 6/16)
...
ggsave(p.perf, file="Figures/Fig4_perf.pdf")
...
s_hop <- Hop_Long %>% filter(!is.na(value)) %>% group_by(Temperature, Photoperiod, Site, Sex) %>% summarise(`10`=sum(TestTemp==10), `17`=sum(TestTemp==17), `25`=sum(TestTemp==25), `35`=sum(TestTemp==35))
#sum(s_hop$N)
s_hop
...
s_feed <- FeedPLmass %>% filter(!is.na(Tot_Consumed_scaled)) %>% group_by(Temperature, Photoperiod, Site, Sex) %>% summarise(`20`=sum(TestTemp==20), `30`=sum(TestTemp==30), `40`=sum(TestTemp==40))
...
hopanova$Significance[hopanova$`Pr(>Chisq)`<.01] <- "** "
...
hopcoeff$Significance[hopcoeff$`Pr...t..`<.01] <- "** "
...
feedcoeff$Significance[feedcoeff$`Pr...t..`<.01] <- "** "
...
#perfcoeff
options(knitr.kable.NA = '')
kable(perfcoeff, digits=3, col.names=c("Coefficient", "Std. Error", "df", "t", "p", "Coefficient", "Std. Error", "df", "t", "p"), escape=FALSE, format="html") %>% kable_styling() %>% add_header_above(c(" " = 1, "Hopping distance (cm)" = 5, "Mass-scaled leaf area consumed (cm²/g, 8h)" = 5)) %>% save_kable("Tables/TabS6_perfcoeff.png")
