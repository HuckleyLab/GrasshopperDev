...
p.hop <- ggplot(data = Hop_Long, aes(x = TestTemp, y = value, col = Site)) +
  ...
  ylab("Hopping distance (cm)") + xlab("Test temperature (°C)") + theme_bw() + theme(strip.background = element_blank()) + coord_cartesian(xlim=c(10, 40))
...
p.feed <- ggplot(data = FeedPLmass, aes(x = TestTemp, y = Tot_Consumed_scaled, col = Site)) +
  ...
  stat_summary(aes(x = as.numeric(TestTemp)), fun = mean, geom = "line", size = .5, position = position_dodge(width = .3)) + scale_color_manual(labels=c("2195m", "2591m", "3048m"), values = brewer.pal(n = 8, name = "Blues")[c(8,6,4)]) +
  #stat_summary(fun.data = Give.N, geom = "text", fun.y = median, position = position_dodge(width = .6), size = 7) +
  ylab("Mass-scaled leaf area consumed (cm2/g, 8h)") + xlab("Test temperature (°C)") + theme_bw() + theme(strip.background = element_blank(), axis.title.y = element_text(size=9)) + coord_cartesian(xlim=c(10, 40))#+ xlim(10,40)

p.perf <- plot_grid(p.hop, p.feed, labels = c('a', 'b'), nrow=2) 
...
```{r}
#fullmodel
hopmodlongtt <- lmer(value ~ TestTemp * I(TestTemp^2) * Sex * Site * Temperature * Photoperiod 
               - (TestTemp:I(TestTemp^2)* Sex * Site * Temperature * Photoperiod) 
               + Sex * Site * Temperature * Photoperiod +
                   #(1|variable/ID/Female2), #original
                   #(1|Female2/ID/variable), #this gave a singularity issue 
                   (1|Female2/ID), #+ (1|variable), #so this did work -- might wanna double check it!
                 #REML = FALSE,
...
```{r}
hopanovalongtt <- Anova(hopmodlongtt, type=3)

hopanovalongtt$Significance <- "   "
hopanovalongtt$Significance[hopanovalongtt$`Pr(>Chisq)`<.05] <- "*  "
hopanovalongtt$Significance[hopanovalongtt$`Pr(>Chisq)`<.001] <- "***"
hopanovalongtt$`Pr(>Chisq)` <- format(hopanovalongtt$`Pr(>Chisq)`, scientific=TRUE, digits=3)
hopanovalongtt$`Pr(>Chisq)` <- as.character(hopanovalongtt$`Pr(>Chisq)`)
hopanovalongtt <- hopanovalongtt %>% unite(p, c(`Pr(>Chisq)`, Significance), sep="")
options(knitr.kable.NA = '')
kable(hopanovalongtt, digits=3, col.names=c("<var>&chi;<sup>2</sup></var>", "df", "p"), escape=FALSE, format="html") %>% kable_styling() #%>% save_kable("hopanovalongtt.png")
```
...
## Feeding... still choosing a model
```{r}
...
a <- a %>% unite(p, c(`Pr(>Chisq)`, Significance), sep="")
options(knitr.kable.NA = '')
kable(a, digits=3, col.names=c("<var>&chi;<sup>2</sup></var>", "df", "p"), escape=FALSE, format="html") %>% kable_styling() #%>% save_kable("feedinganovafull.png")
```
```{r}
