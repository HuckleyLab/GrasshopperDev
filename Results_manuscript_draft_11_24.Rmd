...
```{r, echo=FALSE}
s <- summary(nLmer0)
kable(s[["coefficients"]], digits=3, format="html") %>% kable_styling()
```
...
# 4 way interaction
plot_model(nLmer0,type="pred",terms=c("Site","Tempvar","Daylength","Sex"),show.data=FALSE)
...
p2 <- plot_model(nLmer0, type="pred", terms=c("Tempvar"), show.data=FALSE)
p3 <- plot_model(nLmer0, type="pred", terms=c("Daylength"), show.data=FALSE)
...
Fig S3
```{r, echo=FALSE}
p1 <- plot_model(nLmer0, type="pred", terms=c("Site", "Tempvar"), show.data=FALSE)
p2 <- plot_model(nLmer0, type="pred", terms=c("Site", "Daylength"), show.data=FALSE)
p3 <- plot_model(nLmer0, type="pred", terms=c("Sex", "Tempvar"), show.data=FALSE)
p4 <- plot_model(nLmer0, type="pred", terms=c("Tempvar", "Daylength"), show.data=FALSE)
...
Lmer0.1 <- lmer(as.numeric(value) ~ sex * Site * Temp * Light +                   (1|Female2/Individual)+(1|variable), na.action = 'na.omit', REML=FALSE, data = Instars_Adult_Long_names) 
...
#3-way interaction
plot_model(LmerM0,type="pred",terms=c("Site", "Temp","Light"),show.data=FALSE)
...
S5
```{r, echo=FALSE}
plot_model(LmerM0,type="pred",terms=c("Site", "Temp","sex"),show.data=FALSE)
...
S6
```{r, echo=FALSE}
plot_model(LmerM0,type="pred",terms=c("Site", "Temp"),show.data=FALSE)
...
lm_site <- lm(as.numeric(CT_max) ~ Site,
                 na.action = 'na.omit', data = CT)

lm_temp <- lm(as.numeric(CT_max) ~ Temp,
                 na.action = 'na.omit', data = CT)

lm_light <- lm(as.numeric(CT_max) ~ Light,
                 na.action = 'na.omit', data = CT)

lm_st <- lm(as.numeric(CT_max) ~ Site + Temp,
                 na.action = 'na.omit', data = CT)

lm_sl <- lm(as.numeric(CT_max) ~ Site + Light,
                 na.action = 'na.omit', data = CT)

lm_tl <- lm(as.numeric(CT_max) ~ Temp + Light,
                 na.action = 'na.omit', data = CT)

lm_sxt <- lm(as.numeric(CT_max) ~ Site * Temp,
                 na.action = 'na.omit', data = CT)

lm_sxl <- lm(as.numeric(CT_max) ~ Site * Light,
                 na.action = 'na.omit', data = CT)

lm_txl <- lm(as.numeric(CT_max) ~ Temp * Light,
                 na.action = 'na.omit', data = CT)

lm_stl <- lm(as.numeric(CT_max) ~ Temp + Light + Site,
                 na.action = 'na.omit', data = CT)

lm_sxtxl <- lm(as.numeric(CT_max) ~ Temp * Light * Site,
                 na.action = 'na.omit', data = CT)
aictab(list(lm_site, lm_temp, lm_light, lm_st, lm_sl, lm_tl, lm_sxt, lm_sxl, lm_txl, lm_stl, lm_sxtxl), modnames = c("lm_site", "lm_temp", "lm_light", "lm_st", "lm_sl", "lm_tl", "lm_sxt", "lm_sxl", "lm_txl", "lm_stl", "lm_sxtxl"))
```

```{r, include=FALSE}
#exploring CT_min without random effects 

lm_site <- lm(as.numeric(CT_min) ~ Site,
                 na.action = 'na.omit', data = CT)

lm_temp <- lm(as.numeric(CT_min) ~ Temp,
                 na.action = 'na.omit', data = CT)

lm_light <- lm(as.numeric(CT_min) ~ Light,
                 na.action = 'na.omit', data = CT)

lm_st <- lm(as.numeric(CT_min) ~ Site + Temp,
                 na.action = 'na.omit', data = CT)

lm_sl <- lm(as.numeric(CT_min) ~ Site + Light,
                 na.action = 'na.omit', data = CT)

lm_tl <- lm(as.numeric(CT_min) ~ Temp + Light,
                 na.action = 'na.omit', data = CT)

lm_sxt <- lm(as.numeric(CT_min) ~ Site * Temp,
                 na.action = 'na.omit', data = CT)

lm_sxl <- lm(as.numeric(CT_min) ~ Site * Light,
                 na.action = 'na.omit', data = CT)

lm_txl <- lm(as.numeric(CT_min) ~ Temp * Light,
                 na.action = 'na.omit', data = CT)

lm_stl <- lm(as.numeric(CT_min) ~ Temp + Light + Site,
                 na.action = 'na.omit', data = CT)

lm_sxtxl <- lm(as.numeric(CT_min) ~ Temp * Light * Site,
                 na.action = 'na.omit', data = CT)
aictab(list(lm_site, lm_temp, lm_light, lm_st, lm_sl, lm_tl, lm_sxt, lm_sxl, lm_txl, lm_stl, lm_sxtxl), modnames = c("lm_site", "lm_temp", "lm_light", "lm_st", "lm_sl", "lm_tl", "lm_sxt", "lm_sxl", "lm_txl", "lm_stl", "lm_sxtxl"))
```
...
(Need to remove "Sex" below for it to run)
<!-- CTmin (will reformat) -->
<!-- ```{r} -->
<!-- #CT -->
